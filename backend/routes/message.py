...
def _message_handlers():
    return MessageHandlers(
        ...
        session_summary_service=current_app.session_summary_service,
    )
...
def save_user_input(email, therapy_session_id):
    return _message_handlers().handle_save_user_input(email=email, therapy_session_id=therapy_session_id, data=request.json)


@message_bp.route('/sessions/<int:therapy_session_id>/messages/ai', methods=['POST'])
@token_required
def get_ai_response(email, therapy_session_id):
    return _message_handlers().handle_ai_response(email=email, therapy_session_id=therapy_session_id, data=request.json)


@message_bp.route('/sessions/<int:therapy_session_id>/messages', methods=['GET'])
@token_required
def get_all_messages(email, therapy_session_id):
    return _message_handlers().handle_get_all_messages(email=email, therapy_session_id=therapy_session_id)


@message_bp.route("/sessions/<int:therapy_session_id>/messages", methods=["DELETE"])
@token_required
def delete_all_messages(email, therapy_session_id):
    return _message_handlers().handle_delete_all_messages(email=email, therapy_session_id=therapy_session_id)
